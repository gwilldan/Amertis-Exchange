...
const History = () => {
    ...
    return (
        <>
            <div className=" bg-yellow-500/10 border-yellow-500/50 border-[1px] rounded-[16px] p-2 md:p-4 mb-5 flex items-center gap-2">
                ...
                <ul className="flex-1 overflow-auto rounded-b-[30px] ">
                    {history?.map((transaction: any) =>
                        <div key={transaction.id} className="grid grid-cols-4 gap-3 items-center my-4">
                            ...
                            <span className="text-right">{new Date(Number(transaction.timeStamp) * 1000).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>
                        </div>
                    )}
                </ul>
            </main>
...
const TokenDisplay = ({ tokenDetails, amount }: { tokenDetails: { decimals: number, id: string, name: string, symbol: string }, amount: string }) => {
    const chainId = useChainId();
    const tokenData = TokenList[chainId][tokenDetails.id]

    const formattedAmount = formatTokenAmount(BigInt(amount), tokenDetails.decimals);
    ...
    return (
        <span className={`flex items-center md:gap-2 `}>
            ...
            <div className="ml-2 md:ml-0">
                <h1 className="">{formattedAmount} {tokenDetails.symbol}</h1>
